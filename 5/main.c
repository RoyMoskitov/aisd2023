...
int main () {
	...
	while (1) {
		int check;
		printf("0 - Quit    1 - Add vertex\n2 - Add edges    3 - Delete vertex\n4 - Delete edge    5 - Change Vertex\n6 - Print tree    7 - Traverse\n8 - Bellman-Ford    9 - Floyd\n10 - Make png\n");
		scanf("%d", &check);
		switch (check) {
			case (0):
				check = 0;
				break;
			case (1):
				if (DAddVertex(tree) == 1) check = 0;
				break;
			case (2):
				if (DAddEdge(tree) == 1) check = 0;
				break;
			case (3):
				if (DDelVertex(tree) == 1) check = 0;
				break;
			case (4):
				if (DDelEdge(tree) == 1) check = 0;
				break;
			case (5):
				if (DChangeVertex(tree) == 1) check = 0;
				break;
			case (6):
				if (DPrint(tree) == 1) check = 0;
				break;
			case (7):
				if (DTraverse(tree) == 1) check = 0;
				break;
			case (8):
				if (DBellmanFord(tree) == 1) check = 0;
				break;
			case (9):
				if (DFloyd(tree) == 1) check = 0;
				break;
			case (10):
				if (D_Png(tree) == 1) check = 0;
				break;
			default:
